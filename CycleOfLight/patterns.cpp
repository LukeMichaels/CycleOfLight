...
uint8_t getPatternCount(){
    return static_cast<uint8_t>(std::min(static_cast<int>(ARRAY_SIZE(patternList)), static_cast<int>(MAXPATTERNS)));
}
...
uint8_t patternSunriseNewHelp1(uint32_t offset){
	param.size = static_cast<uint16_t>(std::min(static_cast<uint16_t>(risen - offset), static_cast<uint16_t>(65500)));
...
void sunriseSingle(uint8_t bar,float h){
	...
	h = std::max(static_cast<decltype(h)>(h), static_cast<decltype(h)>(0));
