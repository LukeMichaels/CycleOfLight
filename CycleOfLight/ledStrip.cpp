...
uint16_t getHue(uint8_t red, uint8_t green, uint8_t blue) {
    uint8_t minValue = min(min(red, green), blue);
    uint8_t maxValue = max(max(red, green), blue);
    ...
    sat = 255 * long(maxValue - minValue) / value;
    if (sat == 0){
        hue = 0;
        return 0;
    }
    ...
    if (maxValue == red){
        hue = 0 + 43 * (green - blue) / (maxValue - minValue);
	}else if (maxValue == green){
        hue = 85 + 43 * (blue - red) / (maxValue - minValue);
	}else{
        hue = 171 + 43 * (red - green) / (maxValue - minValue);
	}
	if(hue > 500){
		hue +=256;
	}
