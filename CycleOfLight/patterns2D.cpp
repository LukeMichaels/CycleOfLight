...
void patternFire2D(){
	...
	*/
	...
	for (uint8_t i = 0; i < std::min(static_cast<int>(8), static_cast<int>(particles)); i++) { // 8 Particles when available
...
// https://rosettacode.org/wiki/Plasma_effect#C
	...
	return getLedColor(
    	static_cast<uint8_t>(std::max(static_cast<int>((255 + darkcorr) * fabs(sin(dv * pi)) - darker), 0)),
    	static_cast<uint8_t>(std::max(static_cast<int>((255 + darkcorr) * fabs(sin(dv * pi + 2 * pi / 3)) - darker), 0)),
    	static_cast<uint8_t>(std::max(static_cast<int>((255 + darkcorr) * fabs(sin(dv * pi + 4 * pi / 3)) - darker), 0)),
    0);
